...
DEBUG_BUILD=( y n )
PLATFORM=( core photon P1 electron xenon argon boron xenon-som argon-som boron-som bg96 )
# P1 bootloader built with gcc 4.8.4 doesn't fit flash, disabling for now
PLATFORM_BOOTLOADER=( core photon electron xenon argon boron xenon-som argon-som boron-som bg96 )
SPARK_CLOUD=( y n )
APP=( "" tinker product_id_and_version)
TEST=( wiring/api wiring/no_fixture )

MODULAR_PLATFORM=( photon P1 electron xenon argon boron xenon-som argon-som boron-som bg96 )
...
do
  ...
  do
    ...
    if [[ "$db" = "y" ]]; then
      if [[ "$p" = "xenon" ]] || [[ "$p" = "argon" ]] || [[ "$p" = "boron" ]] || [[ "$p" = "xenon-som" ]] || [[ "$p" = "argon-som" ]] || [[ "$p" = "boron-som" ]] || [[ "$p" = "bg96" ]]; then
        continue
      fi
    fi
